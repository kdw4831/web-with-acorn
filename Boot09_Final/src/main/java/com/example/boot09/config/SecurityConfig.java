...
public class SecurityConfig {
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception{
		...
		String[] whiteList= {"/", "/user/signup_form", "/user/signup", 
				...
				"/gallery/list", "/upload/images/**","/gallery/detail","/cafe/list","cafe/detail"};
		...
		.authorizeHttpRequests(config -> 
			...
			.requestMatchers("/admin/**").hasRole("ADMIN") //admin 하위는 admin 
			.requestMatchers("/staff/**").hasAnyRole("ADMIN","STAFF")//staff하위는 admin, staff
			.anyRequest().authenticated()// 위에 명시한 이외의 모든 요청은 로그인해야지 요청가능하게
		)// /, /play 요청은  받아들이고  그외 어떤 요청은 인증을 하겠다.
		.formLogin(config->
			config
				//인증을 거치지 않은 사용자를 리다일렉트 시킬 경로 설정
			    .loginPage("/user/required_loginform")
				//로그인 처리를 할 때 요청될 url 설정( spring security가 login 처리를 대신 해준다.)
		...
		.logout(config->
			config
				.logoutUrl("/user/logout")//Spring Security가 자동으로 로그아웃 처리 해줄 경로 설정
		...
		.sessionManagement(config ->
			config
			.maximumSessions(1)  // 최대 허용 세션 갯수( 로그인 가능 기기 1개)
