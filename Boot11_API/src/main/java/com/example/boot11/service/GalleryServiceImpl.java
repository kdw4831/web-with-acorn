...
//		model.addAttribute("pageNum",pageNum);
	...
	public void deleteOne(int num) {
		//삭제할 글의 정보를 얻어온다.
		GalleryDto dto =dao.getdata(num);
		//로그인된 userName을 읽어온다. (로그인 하지 않은 경우 null 일수도 있다.)
		String userName=SecurityContextHolder.getContext().getAuthentication().getName();
		//로그인된 사용자와 글의 작성자가 같은지 확인해서 같지 않으면 Exception을 발생시킨다.
		if(!dto.getWriter().equals(userName)) {
			//예외를 발생시킨다.
			//throw new RuntimeException("남의 파일 지우면 혼난다!");//이건 스프링이 
			//throw new NotOwnerException("이건 당신의 소유가 아닙니다");// 이건 내가 exception을 만드는것
		}	
		//사진을 파일 시스템에서 삭제
